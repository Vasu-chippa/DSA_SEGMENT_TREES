...
public class kth {
    ...
    public static int query(int index,int st,int ed,int i,int j) {
        ...
        if(i<=st && ed<=j)
            return tree[index];
        int mid = (st+ed)/2;
        return query(2*index+1,st,mid,i,j) +  query(2*index+2,mid+1,ed,i,j);
    }
    static int kthone(int index, int st, int ed, int k) {
        if(st==ed) return st;
        int mid = (st+ed)/2;
        if(tree[2*index+1]>=k)
            return kthone(2*index+1,st,mid,k);
        else
            return kthone(2*index+2,mid+1,ed,k-tree[2*index+1]);
    }
	public static void main(String[] args)  {
		...
		for(int i=0;i<Q;i++) {
		    ...
		    if(type==1)  {
		        ...
		        int k = sc.nextInt();
		        if(k> tree[0]) System.out.println("-1");
                else {
                    int st = 0;
                    int ed = n-1;
		            System.out.println(kthone(0,st,n-1,k));
                }
		    }
		}
	}
